import React from "react";
import Collapse from "../../components/Collapse";
import { Link, ScrollRestoration } from "react-router-dom";
import ScrollToHashElement from "../../components/ScrollToHashElement";
import { useTranslation } from "react-i18next";

interface Protocol {
  title: string;
  link: string;
}

interface SubSection {
  title: string;
  protocols: Protocol[];
  image: string;
}

interface Section {
  title: string;
  id: string;
  subSections: SubSection[];
}

const ProtocolIntro = () => {
  const { t } = useTranslation("protocolIntro");

  const getSections = (): Section[] => {
    const sections = t("sections", { returnObjects: true }) as Section[];
    return Array.isArray(sections) ? sections : [];
  };

  const sections = getSections();

  return (
    <div className="flex flex-col xl:px-64">
      <h1 className="py-8 text-3xl font-bold">{t("title")}</h1>
      <div className="w-full">
        <div className="flex flex-row flex-wrap justify-center gap-3 text-xl align-center text-physioBlue">
          {sections.map((section, index) => (
            <div key={section.id} className="flex flex-row py-4 pb-8">
              <a href={`#${section.id}`} className="hover:text-blue-300">
                <p> {section.title} </p>
              </a>
              {index !== sections.length - 1 && <p className="ml-2">|</p>}
            </div>
          ))}
        </div>

        {sections.map((section) => (
          <Collapse key={section.id} title={section.title}>
            <span
              id={section.id}
              className="absolute"
              style={{ top: "-150px" }}
            />

            {section.subSections.map((subSection, subIndex) => (
              <Collapse
                key={`${section.id}-${subIndex}`}
                subtitle
                title={subSection.title}
              >
                <div className="flex flex-col items-center md:flex-row">
                  <img
                    src={subSection.image}
                    className="py-4 rounded-[50px] max-w-[269px]"
                    alt={subSection.title}
                  />
                  <div className="flex flex-col gap-3 px-4 lg:flex-wrap">
                    {subSection.protocols.map((protocol, protocolIndex) => (
                      <Link
                        key={`${section.id}-${subIndex}-${protocolIndex}`}
                        to={protocol.link}
                      >
                        <li className="ml-2 text-xl leading-8 underline hover:text-blue-300 text-physioBlue hover:cursor-pointer">
                          {protocol.title}
                        </li>
                      </Link>
                    ))}
                  </div>
                </div>
              </Collapse>
            ))}
          </Collapse>
        ))}
      </div>
      <ScrollRestoration />
      <ScrollToHashElement />
    </div>
  );
};

export default ProtocolIntro;
